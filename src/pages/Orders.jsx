...
const generateDummyOrders = () => {
    ...
    const cities = ['Phnom Penh', 'Siem Reap', 'Battambang', 'Sihanoukville', 'Kampong Cham'];

    return Array.from({ length: 25 }, (_, i) => {
        const itemCount = (i % 3) + 1;
        const items = Array.from({ length: itemCount }, (_, j) => allProducts[(i * 3 + j) % allProducts.length].name);
        const total = items.reduce((sum, name) => {
            const p = allProducts.find(pr => pr.name === name);
            return sum + (p ? p.price : 40);
        }, 0);
        return {
            id: `#RM-${String(1092 + i)}`,
                        ...
                        <tbody className="text-sm divide-y divide-gray-100">
                            {currentOrders.map((order) => (
                                <tr key={order.id} className="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td className="px-4 py-2.5 text-gray-500">{order.city}</td>
                                    <td className="px-4 py-2.5 text-gray-500 max-w-[220px] truncate">{order.items}</td>
