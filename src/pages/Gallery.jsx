...
const buildGalleryFromProducts = () => {
    const images = [];
    allProducts.forEach((p) => {
        images.push({ id: `${p.id}-main`, url: p.img, title: p.name });
        (p.thumbs || []).forEach((t, i) => {
            images.push({ id: `${p.id}-thumb-${i}`, url: t, title: `${p.name} (alt)` });
        });
    });
    return images;
};
...
const Gallery = () => {
    const [images, setImages] = useState(buildGalleryFromProducts());
